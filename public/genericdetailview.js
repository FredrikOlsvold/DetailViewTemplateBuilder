(function () {
    ...
    */
    ...
    class TemplateRetriever {
        ...
        async getTemplateFromTemplateId(templateId) {
            if (!this.templates.has(templateId)) {
                this.templates.set(templateId, QueryServer2(new GetTemplateDataQuery({
        ...
        async getTemplateId(scopeId, dataStreamid, dataItemType) {
            return await QueryServer2(new GetDetailViewTemplateIdQuery({
                ScopeId: scopeId,
    ...
    if (!LitElement.prototype.addVisibilityChangedObserver) {
        LitElement.prototype.addVisibilityChangedObserver = function (element, visibilityChangedCallback) {
            let observer;
    ...
    */
    ...
    class TextService {
        ...
        getTextOneLine(data, field) {
            ...
            let dataSpan = DataFormatter$1.encapsulateTextInSpanWithClass(tempData, "text " + field.Format);
        ...
        getTextTwoLines(data, field) {
            ...
            if (isMultiline && (isNumber || isEmpty)) {
                ...
                dataSpan = DataFormatter$1.encapsulateTextInSpanWithClass(tempData, "text");
                text = html `${labelSpan} <div class="textContainer ${textClass}" style = "max-height:${styling}em" id = "${textClass}" > ${dataSpan}</div> `;
            }
            else {
                dataSpan = DataFormatter$1.encapsulateTextInSpanWithClass(tempData, "text " + formatting);
        ...
        getValueWithDenomination(data, field) {
            let tempData = this._dataHelper.getDataFromField(data, field);
            let dataSpan = DataFormatter$1.encapsulateTextInSpanWithClass(tempData, field.Format);
            let labelSpan = this._dataHelper.toggleLabel(field);
        ...
        getSplitStringOnFirstOccurenceSelectSpecificPart(data, field, item) {
            try {
                ...
                let output = html `${this._dataHelper.toggleLabel(field)} ${DataFormatter$1.encapsulateTextInSpanWithClass(text, "text")}`;
                return (DataValidator.isEmpty(this._dataHelper.getDataFromField(data, field)) && field.Label.includes("!")) ? html `` : DataFormatter$1.encapsulateFieldTypeOutputInSpan(output, field);
            }
            catch (error) {
                return html ``;
            }
        ...
        getConcattedValues(data, field) {
            ...
            let input = html `${this._dataHelper.toggleLabel(field)}${DataFormatter$1.encapsulateTextInSpanWithClass(text, "text")}`;
        ...
        getStringReplaceWithData(data, field) {
            try {
                ...
                let text = html `${this._dataHelper.toggleLabel(field)}${DataFormatter$1.encapsulateTextInSpanWithClass(stringToBeUpdated, "text")}`;
                return DataFormatter$1.encapsulateFieldTypeOutputInSpan(text, field);
            }
            catch (error) {
                console.log(error);
        ...
        getValueInComplexObject(data, field) {
            ...
            result = html `${this._dataHelper.toggleLabel(field)} ${DataFormatter$1.encapsulateTextInSpanWithClass(result, "textcontent " + field.Format)}`;
    ...
    function tokenize(query, shouldAssignParamIds) {
        if (!query)
            return [];
        var result = [], char, nextChar = query.charAt(0), bStart = 0, bEnd = 0, partOffset = 0, pos = 0, depth = 0, mode = 'get', deepQuery = null;
    ...
    class DataFormatter {
        static getFormattedValue(data, field) {
            ...
            if (this.isEmptyValueDescriptor(field.ValueDescriptor))
                return null;
            // Retrieve data
    ...
    class TableFormatter {
        static renderTable(data, section) {
            ...
            return html ` <table>
                        <thead>
                             ${repeat(resultTable.header, r => html `<tr>${repeat(r, c => html `<th>${c}</th>`)} </tr>`)}
                        </thead>
                        <tbody id="table-items">    
                             ${repeat(resultTable.body, r => html `<tr>${repeat(r, c => html `<td>${c}</td>`)} </tr>`)}     
                        </tbody>
                        ${resultTable.footer.length ? html `
                        <tfoot>
                             ${repeat(resultTable.footer, r => html `<tr>${repeat(r, c => html `<td>${c}</td>`)} </tr>`)}
                        </tfoot>  ` : html ``}
    ...
    class DataRetriever {
        ...
        outputValidator(data, field, input) {
            try {
                ...
                return this.isEmptyValue(tempData, field.Label) ? html `` : DataFormatter$1.encapsulateFieldTypeOutputInSpan(input, field);
            }
            catch (error) {
                console.log(`Error in field type ${field.Type}, check that label: %c${field.Label}%c and value: %c${field.Value}%c are correct.`, "color: red", "color: black", "color: red", error);
                console.log(error.message);
    ...
    class TemplateEngine {
        ...
        renderTemplateFieldsNew(data, fields) {
            return html `${repeat(fields, (f) => f.Id, f => {
            try {
        ...
        renderTemplateFields(data, fields) {
            return html `${repeat(fields, (f) => f.Id, f => this.FieldWriter_TOBEDISCONTINUED[f.Type](data, f))}`;
        }
    ...
    __decorate([
        property()
    ], GenericDetailView.prototype, "data", void 0);
    __decorate([
        property()
    ], GenericDetailView.prototype, "template", void 0);
    __decorate([
        property({ type: String })
    ], GenericDetailView.prototype, "viewClass", void 0);
    __decorate([
        property({ type: String })
    ], GenericDetailView.prototype, "stylingPath", void 0);
    __decorate([
        property()
    ], GenericDetailView.prototype, "loadExternalData", void 0);
    __decorate([
        property()
    ], GenericDetailView.prototype, "templateId", void 0);
    __decorate([
        property()
    ], GenericDetailView.prototype, "openPopUpExternalData", void 0);
    __decorate([
        cancelclick()
    ], GenericDetailView.prototype, "firstUpdated", null);
